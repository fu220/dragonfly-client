...
pub struct CacheTask {
    ...
    /// piece_count is the count of the pieces.
...
impl CacheTask {
    ...
    /// cache_task_length returns the actual memory usage of the cache task.
    pub fn cache_task_length(&self) -> Option<u64> {
        self.piece_count
            .and_then(|count| self.piece_length.map(|len| count * len))
    }
}
...
impl<E: StorageEngineOwned> Metadata<E> {
    ...
    /// update_cache_task_piece_count updates the piece count of the cache task.
    #[instrument(skip_all)]
    pub fn update_cache_task_piece_count(&self, id: &str, piece_count: u32) -> Result<()> {
        let task = match self.db.get::<CacheTask>(id.as_bytes())? {
            Some(mut task) => {
                task.piece_count = Some(task.piece_count.unwrap_or(0) + piece_count as u64);
                task
            }
            None => return Err(Error::TaskNotFound(id.to_string())),
        };

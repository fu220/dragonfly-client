...
impl Storage {
    ...
    async fn handle_downloaded_cache_piece_from_source_finished<R: AsyncRead + Unpin + ?Sized>(
        ...
        if let Err(err) = self.metadata.update_cache_task_piece_count(task_id, 1) {
            error!("update cache task piece count failed: {}", err);
        }
    ...
    /// cache_capacity returns the capacity of the cache.
    pub fn cache_capacity(&self) -> u64 {
        self.cache.capacity()
    }

    /// cache_used_size returns the used size of the cache.
    pub fn cache_used_size(&self) -> u64 {

...
use crate::shutdown;
use dragonfly_api::common::v2::{PersistentCacheTask, Priority, Task, CacheTask, TaskType};
use dragonfly_api::dfdaemon::v2::{
    ...
    StatCacheTaskRequest as DfdaemonStatCacheTaskRequest, 
};
...
use dragonfly_client_util::{
    ...
    id_generator::{PersistentCacheTaskIDParameter, TaskIDParameter, CacheTaskIDParameter},
};
...
impl DfdaemonDownloadClient {
    ...
    /// stat_cache_task gets the status of the cache task.
    #[instrument(skip_all)]
    pub async fn stat_cache_task(&self, request: DfdaemonStatCacheTaskRequest) -> ClientResult<CacheTask> {
        let request = Self::make_request(request);

...
use crate::resource::{persistent_cache_task, task ,cache_task};
use crate::shutdown;
use bytesize::MB;
use dragonfly_api::common::v2::{
    Host, Network, PersistentCacheTask, Piece, Priority, Task, CacheTask, TaskType,
};
use dragonfly_api::dfdaemon::v2::{
    ...
    DeletePersistentCacheTaskRequest, DeleteTaskRequest, DeleteCacheTaskRequest, DownloadPersistentCachePieceRequest,
    DownloadPersistentCachePieceResponse, DownloadPersistentCacheTaskRequest,
    DownloadPersistentCacheTaskResponse, DownloadPieceRequest, DownloadPieceResponse, DownloadCachePieceRequest, DownloadCachePieceResponse, 
    DownloadTaskRequest, DownloadTaskResponse, DownloadCacheTaskRequest, DownloadCacheTaskResponse, ExchangeIbVerbsQueuePairEndpointRequest,
    ExchangeIbVerbsQueuePairEndpointResponse, StatPersistentCacheTaskRequest, StatTaskRequest, StatCacheTaskRequest,
    SyncHostRequest, SyncPersistentCachePiecesRequest, SyncPersistentCachePiecesResponse,
    SyncPiecesRequest, SyncPiecesResponse, SyncCachePiecesRequest, SyncCachePiecesResponse, UpdatePersistentCacheTaskRequest,
};
...
use dragonfly_client_util::{
    ...
    id_generator::{TaskIDParameter, CacheTaskIDParameter},
...
impl DfdaemonUpload for DfdaemonUploadServerHandler {
    ...
    #[instrument(skip_all, fields(host_id, task_id, peer_id, url, remote_ip, content_length))]
    ...
    #[instrument(skip_all, fields(host_id, task_id, remote_ip, local_only))]
    async fn stat_cache_task(&self, request: Request<StatCacheTaskRequest>) -> Result<Response<CacheTask>, Status> {
        // If the parent context is set, use it as the parent context for the span.
        if let Some(parent_ctx) = request.extensions().get::<Context>() {
            Span::current().set_parent(parent_ctx.clone());
        };
    ...
    #[instrument(skip_all, fields(host_id, remote_host_id, task_id, piece_id, piece_length))]
    async fn download_cache_piece(
        ...
        // Generate the piece id.
        let piece_id = self.cache_task.piece.cache_id(task_id.as_str(), piece_number);
    ...
    async fn download_persistent_cache_piece(
        ...
        // Generate the piece id.
        let piece_id = self.persistent_cache_task.piece.persistent_cache_id(task_id.as_str(), piece_number);

...
use dragonfly_api::dfdaemon::v2::{
    download_task_response, DownloadTaskRequest, ListTaskEntriesRequest, DownloadCacheTaskRequest,
    download_cache_task_response,
};
    ...
    if args.cache {
        ...
        while let Some(message) = out_stream.message().await.inspect_err(|err| {
            ...
            match message.response {
                Some(download_cache_task_response::Response::DownloadCacheTaskStartedResponse(response)) => {
                    progress_bar.set_length(response.content_length);
                }
                Some(download_cache_task_response::Response::DownloadPieceFinishedResponse(response)) => {
        ...
        .await
        .inspect_err(|err| error!("download task failed: {}", err))?;
